...
request.addEventListener("load",()=>
{
    ...
    for(let i=0;i<data.length;i++)
    {
        ...
        item.setAttribute("id",data[i][0].product_id);
        let img=document.createElement("img");
        let img1=data[i][0].image;
        img.setAttribute("src",`profile/${img1}`);
        item.appendChild(img);
        let p1=document.createElement("p");
        p1.innerHTML=data[i][0].productname;
        item.appendChild(p1)
        let p2=document.createElement("p");
        p2.innerHTML=data[i][0].price;
        let pri=data[i][0].price.split(".");
        pri=pri[1];
        
        item.appendChild(p2);
        let p3=document.createElement("p");
        p3.innerHTML=data[i][0].details;
        ...
        quantity.innerHTML=data[i][1].curr;
        sub_total=sub_total+(pri*data[i][1].curr);
